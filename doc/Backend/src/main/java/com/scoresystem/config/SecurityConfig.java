	...
	public static class ProductionSecurityConfig extends WebSecurityConfigurerAdapter {
		...
		protected void configure(HttpSecurity http) throws Exception {
			http
					...
					// 允许访问项目列表和其他公共API
					.antMatchers("/projects", "/projects/**").permitAll().antMatchers("/users", "/users/**").permitAll()
					.antMatchers("/tasks", "/tasks/**", "/tasks/active").permitAll().antMatchers("/statistics/**")
					.permitAll()
					// 允许访问评分API
					.antMatchers("/scores", "/scores/**").permitAll().antMatchers("/test-data/**").permitAll()
					// 其他请求需要认证
